...
Now, we will use `read.csv()` to load into memory the content of the CSV file as 
...
> the help for `read.csv()` by typing `?read.csv` to learn more. There is also 
> the `read.delim()` for in tab separated data files. It is important to note that 
> all of these functions are actually wrapper functions for the main `read.table()` > function with different arguments.
>
> As such, the surveys data above could have also been loaded by 
> using `read.table()` with the separation argument as `,`. 
> The code is as follows: `surveys <- read.table(file="data/portal_data_joined.csv", sep=",", header=TRUE)`.
...
mid_row <- surveys[nrow(surveys)/2, ]
mid_row
head_row <- surveys[-c(7: nrow(surveys)),]
head_row
```
## Factors
...
levels(sex)
plot(sex)
sex <- factor(sex, levels = c("male","female","undetermined"))
plot(sex)
```
...
>         added `stringsAsFactors = FALSE` when creating the data frame?
