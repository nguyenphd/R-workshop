...
We have seen in our previous lesson that when building or importing a data frame, 
the columns that contain characters (i.e., text) are coerced (=converted) into the 
`factor` data type. We had to set **`stringsAsFactors`** to **`FALSE`** to avoid 
...
like plots or aquaria. Moving back and forth between these formats is nontrivial, 
and **`tidyr`** gives you tools for this and more sophisticated  data manipulation.
...
```{r load_surveys, purl = FALSE}
surveys <- read_csv("data/portal_data_joined.csv")
```

Let's see what the format of our data is using str() - what does str() stand for?

```{r surveys_structure, purl=FALSE}
str(surveys)
```

Preview the data using view(), or by going ot the Environment tab and clicking the table icon to the left of the surveys data object.

```{r, eval=FALSE, purl=FALSE}
view(surveys)
```

From the str() command, notice that the class of the data is now `tbl_df`

This is referred to as a "tibble". Tibbles tweak some of the behaviors of the data frame objects we 
introduced in the previous episode. The data structure is very similar to a data frame. For our 
...
```{r select_by_exclusion, purl = FALSE}
...
(Note: remember back ot the Math Trick example!)
...
This is readable, but can clutter up your workspace with lots of objects that you have to name
individually. With multiple steps, that can be hard to keep track of.
...
> ### Challenge 5.1 - Subsetting using select() and filter()
>
...
> ### Challenge 5.2 - Modifying a data frame with mutate()
>
...
`group_by()` takes as arguments the column names that contain the **categorical** variables for 
which you want to calculate the summary statistics. `group_by()` is often used together with 
`summarize()`, which collapses each group into a single-row summary of that group.
...
When grouping both by `sex` and `species_id`, the first few rows are for animals
...
Number") **(What's the difference?)** To avoid this, we can remove the missing values for weight before we
...
For convenience, `count()` provides the `sort` argument, which when set to `TRUE` will sort the 
...
order ofthe levels of the species and (ii) in descending order of the count:
...
ofthe *albigula* species that are not specified for its sex (i.e. `NA`).

> ### Challenge 5.3 - Working with count():
...
Check out Chapter 12 of [R For Data Science](https://r4ds.had.co.nz/tidy-data.html#spreading-and-gathering) for more background.
...
practical terms this means the values of the species in `genus` would become the names of column 
variables and the cells would contain the values of the mean weight observed on each plot.
...
and `gather()`. (Check out Chapter 12 of [R For Data Science](https://r4ds.had.co.nz/tidy-data.html#spreading-and-gathering) for more background.)
...
multiple rows, 196 observations of 3 variables. (Hint: how can we check that?)
...
Now, let's create the dataset for exporting. 
Start by removing observations for which the `species_id`, `weight`, `hindfoot_length`, or `sex` data 
are missing:
...
Now remove rare species in two steps. First, make a list of species which appear at least 50 times in our dataset (`species_counts`):
...
Second, keep only those species filtered and contained in `species_counts` and put them in a new 
data frame called `surveys_complete` :
